...
export default class {
  ...
  async insert(req: Request, res: Response, next: NextFunction) {
    const { amount, deptorGuid, avalGuid, currentRate, paymentDate, balance } =
      req.body;

    if (deptorGuid.trim() === avalGuid.trim()) {
      const err: IError = new Error(
        "User and endorsement have the same identifier"
      );
      err.status = 411;
      return next(err);
    }
    ...
    const loanResult = await new LoanFactory().create(
      amount,
      deptorGuid,
      avalGuid,
      currentRate,
  ...
  async delete(req: Request, res: Response, next: NextFunction) {
    ...
    if (dataResult.isErr()) {
      ...
      const result = new LoanDeleteMapping().execute(
  ...
  async list(req: Request, res: Response) {
    ...
    const result = new LoanListMapping().execute(
      list.map((loan) => loan.properties())
    );
    res.json(result);
  }
  ...
  async update(req: Request, res: Response, next: NextFunction) {
    ...
    const dataResult = await this.application.update(
      parseInt(id),
      fieldsToUpdate
    );
    if (dataResult.isErr()) {
    ...
    const result = new LoanUpdateMapping().execute(
      dataResult.value.properties()
    );
    return res.json(result);
  }
